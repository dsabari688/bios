  ...
  async function walk(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (SKIP_DIRS.has(entry.name as unknown as string)) continue;
        await walk(join(dir, entry.name as unknown as string));
      } else if (entry.isFile() && (entry.name as unknown as string) === 'migration.json') {
        out.push(join(dir, entry.name as unknown as string));
      }
    }
  }
