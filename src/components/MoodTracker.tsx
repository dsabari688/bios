...
export default function MoodTracker({ token }: MoodTrackerProps) {
  ...
  useEffect(() => {
    // 1. Load mood history from localStorage
    const localHistory = localStorage.getItem("lifeos_mood_history");
    let historyData: MoodEntry[] = [];
    ...
    if (localHistory) {
      ...
      // Create seed history data for the demo
      const getPastFormattedDate = (daysAgo: number, hour: number, minute: number): string => {
        const d = new Date();
      ...
      historyData = [
        {
          id: "mood_seed_1",
          mood: "Normal",
          note: "Reviewed machine learning telemetry graphs",
          createdAt: getPastFormattedDate(1, 16, 30)
        },
        {
          id: "mood_seed_2",
          mood: "Low",
          note: "Rescheduled core tactical branches twice",
          createdAt: getPastFormattedDate(2, 9, 15)
        },
        {
          id: "mood_seed_3",
          mood: "Great",
          note: "Database sync complete, everything functional",
          createdAt: getPastFormattedDate(3, 10, 0)
        }
      ];
      localStorage.setItem("lifeos_mood_history", JSON.stringify(historyData));
    }

    setHistory(historyData);

    // 2. Check if a mood was already logged today
  ...
  const handleSaveMood = async () => {
    ...
    const formattedDate = formatMoodDate(new Date());
    ...
    // Backend sync in the background if token exists
    if (token) {
      try {
